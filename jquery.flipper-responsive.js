jQuery(function($){
  ...
  $.fn.flipper = function(action, options) {
    ...
    var settings = $.extend({
      ...
      template: $flipper.data('template') || 'dd:HH:ii:ss',
      labels: $flipper.data('labels') || '',
      preload: true
    }, options );
    if(action === 'init'){
    ...
    function setFlipperDate($flipper, dateString){
      ...
      $flipper.find('.flipper-dd').find('.flipper-digit:eq(0) .digit-face:contains(' + days_str[0] + ')').addClass('active'); 
      $flipper.find('.flipper-dd').find('.flipper-digit:eq(1) .digit-face:contains(' + days_str[1] + ')').addClass('active');
